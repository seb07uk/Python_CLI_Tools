...
def download_file(url):
    try:
        ...
        print(f"{Color.YELLOW}[DOWNLOAD]{Color.RESET} Pobieranie: {url}...")
        with urllib.request.urlopen(url) as response, open(local_filename, 'wb') as out_file:
            shutil.copyfileobj(response, out_file)
        print(f"{Color.GREEN}[OK]{Color.RESET} Zapisano w Downloads.")
...
@command(name="browse", aliases=["web"])
def browse(url=None):
    """psBrowser CLI: [Nr] Link, [S(nr)] Szukaj, [h] Historia, [snap] Screenshot, [u] Wstecz."""
    history_stack = []
    ...
    while current_url and current_url.lower() not in ['q', 'quit']:
        ...
        print(f"{Color.CYAN}NAV:{Color.RESET} [Nr] Link | [S(nr) tekst] | [h] history | [about] info | [u] wstecz | [g] google | [q] exit")
        ...
        if cmd.lower() == 'snap':
            ...
            print(f"Zapisano: {fname}"); input("Enter..."); continue
