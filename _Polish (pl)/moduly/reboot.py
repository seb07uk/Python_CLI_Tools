...
__category__ = "system"
__group__ = "core"
__desc__ = "System Reboot Utility - schedule or abort system restart"
...
@command(name="reboot", aliases=["restart", "rb"])
def run(*args):
    """
    Schedule system reboot or abort scheduled reboot.
    Usage: reboot [t<seconds|HH-MM-SS>] | reboot abort | reboot help
    """
    # Help handler
    ...
    # No arguments - restart immediately
    if not args:
        reboot_system(1)
        return
    ...
    if arg.startswith("t"):
        ...
        if "-" in val:
            ...
            print(f"{Color.YELLOW}Restart zaplanowany na {val.replace('-', ':')} (za {sec}s).{Color.RESET}")
            reboot_system(sec)
        elif val.isdigit():
            sec = int(val)
            print(f"{Color.YELLOW}Restart za {sec} sekund.{Color.RESET}")
            reboot_system(sec)
        else:
            print(f"{Color.RED}[!] Błąd: Nieprawidłowy format argumentu '{arg}'{Color.RESET}")
    else:
        print(f"{Color.RED}[!] Nieznany argument. Wpisz 'reboot help' dla pomocy.{Color.RESET}")
