...
# Poprawa ostrości czcionek na Windows
try:
...
class RacerCLI:
    ...
    def show_main_menu(self):
        ...
        buttons = [
            ("NOWA GRA", self.start_game),
            ("HI-SCORES", self.show_high_scores),
            ("POMOC", self.show_help),
            ("ZAMKNIJ", self.root.quit)
        ]
    ...
    def show_help(self):
        for w in self.root.winfo_children(): w.destroy()
        tk.Label(self.root, text="POMOC I OPIS", fg="#0f0", bg=self.bg_color, font=("Segoe UI", 30, "bold")).pack(pady=20)
        
        description = (
            "Witaj w Racer CLI – dynamicznej grze zręcznościowej w klimacie retro!\n"
            "Wcielasz się w kierowcę cyfrowego bolidu, który pędzi przez niekończący\n"
            "się strumień danych. Twoim zadaniem jest przetrwać jak najdłużej."
        )

        help_text = (
            "STEROWANIE:\n"
            "• [A] lub [Strzałka Lewo]  - Skręt w lewo\n"
            "• [D] lub [Strzałka Prawo] - Skręt w prawo\n\n"
            "ZASADY:\n"
            "• Unikaj czerwonych bloków – kolizja kończy bieg.\n"
            "• Punkt zdobywasz za każdy minięty obiekt.\n"
            "• Co 8 punktów następuje przyspieszenie."
        )
        ...
        tk.Button(self.root, text="POWRÓT", command=self.show_main_menu, width=15, 
    ...
    def show_high_scores(self):
        ...
        if not self.scores_list:
            tk.Label(self.root, text="BRAK WYNIKÓW", fg="gray", bg=self.bg_color, font=self.font_text).pack(pady=20)
        else:
    ...
    def game_over(self):
        ...
        if is_top:
            name = simpledialog.askstring("Nowy Rekord!", f"Twój wynik: {self.score}\nPodaj imię:", parent=self.root)
